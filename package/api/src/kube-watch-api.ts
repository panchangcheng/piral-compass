...
import { autobind, EventEmitter, interval } from "piral-compass-utils";
...
import { hackCodeHost, pathRewrite } from "./json-api"
...
export class KubeWatchApi {
  ...
  protected getQuery(): Partial<IKubeWatchRouteQuery> {
    const { isClusterAdmin, allowedNamespaces } = store.get('u_config');
    return {
      api: this.activeApis.map(api => {
        if (isClusterAdmin) return api.getWatchUrl();
        return allowedNamespaces.map((namespace: string) => api.getWatchUrl(namespace))
      }).flat()
    }
  }
  ...
  protected connect() {
    ...
    this.evtSource = new EventSource(pathRewrite(hackCodeHost + this.apiUrl + "?" + stringify(query)),{
      headers: {
