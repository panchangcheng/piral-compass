...
import { EventEmitter, cancelableFetch } from "piral-compass-utils";

const URI = require("urijs");

export const rewriteDict: { [key: string]: string } = {
  "api-kube": "workload",
  "api-resource": "workload",
  "api": "workload"
}
export const hackCodeHost = "http://10.200.65.195:8080"
...
export function pathRewrite(url:string) {
  let uri = URI(url);
  const segment0 = uri.segment(0);
  if (rewriteDict.hasOwnProperty(segment0)) { uri.segment(0, rewriteDict[segment0]) }
  return uri.toString()
}
...
export class JsonApi<D = JsonApiData, P extends JsonApiParams = JsonApiParams> {
  ...
  protected request<D>(path: string, params?: P, init: RequestInit = {}) {
    let reqUrl = pathRewrite(hackCodeHost + this.config.apiPrefix + path);
